...
test("should reject on domain name without romanian tld", () => {
  expect.assertions(1);
  return expect(queryRoTLDWhoisServer("example.com")).rejects.toBeInstanceOf(
    Error
  );
});

test("should resolve with whois data on domain name with romanian tld", () => {
  return expect(queryRoTLDWhoisServer("rotld.ro")).resolves.toMatch(
...
test.skip("should reject on  connection error", () => {
  expect.assertions(1);
  return expect(queryRoTLDWhoisServer("rotld.ro")).rejects.toBeInstanceOf(
    Error
  );
});
